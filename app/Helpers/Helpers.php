...
if (!function_exists('send_otb_whatsapp')) {
    /**
     * Get icon
     *
     * @param $path
     ...
     * @return string
     */
    function send_otb_whatsapp($phone)
    {
        ...
        DB::table('password_reset_tokens')->insert([
            'phone' => $phone,
            'token' => $otp,
            'created_at' => now(), // وقت الإنشاء الحالي
        ]);
        ...
        try {
            ...
            $twilio->messages
                ->create("whatsapp:" . str_replace(' ', '', $phone), // to
                    array(
            ...
            return \view('pages.auth.verify-code', ['phone' => $phone, 'token' => $otp]);

        } catch (TwilioException $e) {

            //Alert::error(__('menu.error'), __('menu.message_send_failed'));
            Alert::error(__('menu.error'),$e->getMessage());
            return back();
